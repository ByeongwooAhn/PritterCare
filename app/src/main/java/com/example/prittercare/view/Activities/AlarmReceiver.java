...
/**
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        String message = "예약이 실행되었습니다: " + (reservationName == null || reservationName.isEmpty() ? "알람" : reservationName);
        ...
        if (reservation == null) {
            Log.e(TAG, "No reservation data provided - 예약 데이터 없음"); // 예약 데이터가 없는 경우 로그 출력
            return; // 처리 중단
        }
        ...
        // MQTT 토픽 설정
        String topic = DataManager.getInstance().getUserName() + "/" + DataManager.getInstance().getCurrentCageSerialNumber() + "/light"; // 토픽 경로
